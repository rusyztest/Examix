...
- В `supabase/seed.sql` уже добавлен набор заданий А1: «Пишется А на месте пропуска в словах».
- Открыть тренировку можно на странице `/practice/a1`.
- Для таких заданий используется `tasks.type = 'multiple'`, а правильный ответ хранится в `answers.answer_text` как набор номеров, например `2,3,4`.
- Проверка принимает ввод `2,3,4`, `2 3 4` или `234`, поэтому правильных вариантов может быть от 2 до 4.
